          ...
          <div className="lg:w-3/5 p-6 space-y-6 border-r">
            <div>
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded-lg p-3 border-[#e2f5f2] opacity-50">
          ...
          <div className="lg:w-2/5 bg-[#f4fffe] p-6 space-y-4">
            <div className="flex gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-1 text-sm"><Star className="w-4 h-4 fill-current text-[#00c9a7]" /><span>{property.rating}</span><span className="text-[#5b6b7a]">({property.reviews})</span></div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
              <div className="border-b pb-4">
                <div className="text-sm text-[#5b6b7a]">Hôtel de luxe · {property.beds} chambres · {property.beds} lits · {property.baths} sdb</div>
              ...
              <div className="bg-[#00c9a7]/10 rounded-xl p-5 flex gap-4 items-center overflow-hidden relative group">
                ...
                <div>
                  ...
                  <div className="text-[#5b6b7a]">Un des hôtels préférés des voyageurs au Bénin</div>
                </div>
              </div>
...
export default function Hotels({ onNavigate }: HotelsPageProps) {
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* En-tête */}
      <div className="sticky top-0 z-20 bg-white border-b border-[#e2f5f2]">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-[#5b6b7a]">Hôtels de qualité supérieure au Bénin.</p>
          </div>
        </div>
      </div>

      {/* Filtres */}
      <div className="border-b border-[#e2f5f2] bg-white sticky top-[73px] z-10">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="relative inline-block">
            <button
              onClick={() => setShowFilterDropdown(!showFilterDropdown)}
              className="flex items-center gap-2 px-4 py-2 rounded-full border border-[#c9f0e8] hover:border-[#00c9a7] transition-colors"
            >
              ...
              <div className="absolute top-full left-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-[#e2f5f2] z-50 py-2">
                {filters.map((filter) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-[#00c9a7]/10 transition-colors ${selectedFilter === filter ? "text-[#00c9a7] font-medium" : "text-[#1c3b56]"}`}
                  >
                    {filter}
                  </button>
                ))}
        ...
        <div className="lg:w-1/2 overflow-y-auto p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {displayedProperties.map((property) => (
              <div
                key={property.id}
                className="flex flex-col gap-3 p-4 border border-[#e2f5f2] rounded-2xl hover:shadow-xl transition-all duration-300 cursor-pointer bg-white group hover:border-[#00c9a7] hover:scale-[1.02]"
                ...
                <div>
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-[#5b6b7a]">{property.location}</p>
                    </div>
                    <div className="flex items-center gap-1 bg-[#e8faf6] px-2 py-1 rounded-full">
                      <Star className="w-3 h-3 fill-current text-[#00c9a7]" />
                      <span className="text-sm font-medium">{property.rating}</span>
                      <span className="text-xs text-[#5b6b7a]">({property.reviews})</span>
                    </div>
                  </div>
                  <p className="text-sm text-[#5b6b7a] mt-2 line-clamp-2">{property.description}</p>
                  <div className="flex items-center gap-4 mt-3 text-sm text-[#5b6b7a]">

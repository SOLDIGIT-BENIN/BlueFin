...
export function AdminUsersPage() {
  ...
  const getHostTypeBadge = (hostType?: string | null) => {
    const badges: Record<string, { label: string; color: string; icon: any }> = {
      'logement': { 
        label: '🏠 Logement', 
        color: 'bg-[#e8faf6] text-[#0f2940] border-[#c9f0e8]',
        icon: Home
      },
      'experience': { 
        label: '🎯 Expérience', 
      ...
      'service': { 
        label: '🔧 Service', 
        color: 'bg-[#fff3cd] text-[#4a3400] border-[#ffe9a8]',
        icon: Briefcase
      }
    };
    return hostType ? badges[hostType] : { 
      label: 'Non défini', 
      color: 'bg-[#e8faf6] text-[#5b6b7a] border-[#e2f5f2]',
      icon: Building2
    };
  };
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 bg-gradient-to-br from-[#f4fffe] to-[#e8faf6] min-h-screen">
      ...
      <div className="mb-6">
        ...
        <p className="text-xs sm:text-sm text-[#5b6b7a] mt-1">Gérez et modérez les utilisateurs de la plateforme</p>
      </div>
      ...
      {/* Nombre de résultats */}
      <div className="mb-3 text-sm text-[#5b6b7a]">
      ...
      <div className="space-y-3">
        {filteredUsers.length === 0 ? (
          <div className="bg-white rounded-xl p-8 text-center">
            <Users className="w-12 h-12 text-[#9fb3c4] mx-auto mb-3" />
            <p className="text-[#5b6b7a] text-sm">Aucun utilisateur trouvé</p>
          </div>
        ) : (
...
const UserCard = ({ user, onView, onSuspend, onActivate }: { user: User; onView: () => void; onSuspend: () => void; onActivate: () => void }) => {
  const roleColors: Record<string, string> = {
    voyageur: 'bg-[#e8faf6] text-[#0f2940]',
    hote: 'bg-green-100 text-green-700',
    admin: 'bg-[#e8faf6] text-[#0f2940]',
  };
  ...
  const getHostTypeDisplay = () => {
    ...
    const badges: Record<string, { label: string; color: string }> = {
      'logement': { label: '🏠 Logement', color: 'bg-[#e8faf6] text-[#0f2940]' },
      'experience': { label: '🎯 Expérience', color: 'bg-[#e8faf6] text-[#0f2940]' },
      'service': { label: '🔧 Service', color: 'bg-[#fff3cd] text-[#6b4e06]' }
    };
    
    if (user.host_type && badges[user.host_type]) {
      return badges[user.host_type];
    }
    return { label: '❓ Non défini', color: 'bg-[#e8faf6] text-[#5b6b7a]' };
  };
  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl p-4 shadow-sm hover:shadow-md transition-all border border-[#e2f5f2]">
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="sm:hidden">
            ...
            <p className="text-xs text-[#5b6b7a]">{user.user_type}</p>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="hidden sm:block">
            ...
            <p className="text-sm text-[#5b6b7a]">{user.user_type === 'hote' ? 'Hôte' : user.user_type === 'voyageur' ? 'Voyageur' : 'Admin'}</p>
          </div>
        ...
        <div className="flex items-center justify-between sm:justify-end gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#5b6b7a] hidden sm:inline">{user.is_active ? 'Actif' : 'Suspendu'}</span>
          </div>
...
const UserDetailModal = ({ user, onClose }: { user: User; onClose: () => void }) => {
  ...
  const getHostTypeInfo = () => {
    ...
    const infos: Record<string, { label: string; color: string; icon: any; description: string }> = {
      ...
      'service': { 
        label: '🔧 Service', 
        color: 'bg-[#fff3cd] text-[#6b4e06] border-[#ffe9a8]',
        icon: Briefcase,
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-1">
                ...
                <div className={`w-2 h-2 rounded-full ${user.is_active ? 'bg-green-500' : 'bg-red-500'}`} />
                <span className="text-xs text-[#5b6b7a]">{user.is_active ? 'Actif' : 'Suspendu'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#f4fffe] rounded-xl p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              <div className="flex justify-between">
                <span className="text-[#5b6b7a]">Nom complet</span>
                <span className="font-medium">{user.first_name} {user.last_name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#5b6b7a]">Email</span>
                <span className="font-mono text-xs">{user.email}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#5b6b7a]">Téléphone</span>
                <span>{user.phone || 'Non renseigné'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#5b6b7a]">Inscrit le</span>
                <span>{new Date(user.created_at).toLocaleDateString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#5b6b7a]">Dernière connexion</span>
                <span>{user.last_login_at ? new Date(user.last_login_at).toLocaleString() : 'Jamais'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#5b6b7a]">Vérifié</span>
          ...
          {user.suspended_until && (
            <div className="bg-[#fffaeb] rounded-xl p-4 border border-[#ffe9a8]">
              ...
              {user.suspension_reason && (
                <p className="text-xs text-[#a87c10] mt-2">Raison : {user.suspension_reason}</p>
              )}
            </div>
          )}
        </div>
...
const StatBadge = ({ label, value, color, icon }: any) => {
  const colors: Record<string, string> = {
    ...
    purple: 'bg-[#e8faf6] text-[#0f2940]',
    orange: 'bg-[#fff3cd] text-[#6b4e06]',
  ...
  <div className="p-3 sm:p-4 md:p-6">
    <div className="animate-pulse">
      ...
      <div className="grid grid-cols-4 gap-2 mb-6">
        {[1, 2, 3, 4, 5, 6, 7].map(i => <div key={i} className="bg-[#e2f5f2] rounded-lg h-16"></div>)}
      </div>
